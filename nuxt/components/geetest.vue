...
export default {
  ...
  methods: {
    async defaultPost () {
      await Object.assign(this.geet, { challenge: this.data.challenge, gt: this.data.gt })
      await this.geetTest(this.geet.challenge, this.geet.gt);
    },
    async geetTest (challenge, gt) {
      const self = this
      const captchaBox = this.$refs.captchaBox
      const captch = await new Promise((resolve) => {
        window.initGeetest({ gt: gt, challenge: challenge, lang: 'zh-cn', product: 'embed', offline: false, new_captcha: false, width: '100%' }, (captch) => resolve(captch))
      })
